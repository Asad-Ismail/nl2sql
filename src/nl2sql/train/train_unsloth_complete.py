...
logging.basicConfig(
    ...
    handlers=[logging.FileHandler("training_unsloth.log"), logging.StreamHandler()],
)
...
def load_and_prepare_datasets(dataset_configs: List[DatasetConfig], tokenizer) -> tuple:
    ...
    for config in dataset_configs:
        try:
            ...
            if config.is_hf_dataset:
                dataset = load_dataset(config.path, data_files=config.data_file, split="train")
    ...
    mixed_dataset = interleave_datasets(
        datasets_list, probabilities=probabilities, seed=42, stopping_strategy="all_exhausted"
    )
...
class ProgressCallback(TrainerCallback):
    ...
    def on_epoch_end(self, args, state, control, **kwargs):
        if self.epoch_start_time:
            ...
            total_epochs = int(args.num_train_epochs) if args.num_train_epochs else 3
            logger.info(
                f"✓ Epoch {epoch_num}/{total_epochs} completed in {epoch_time/60:.1f} minutes"
            )
        ...
        if logs and state.global_step > 0:
            if state.max_steps > 0:
                ...
                if self.start_time:
                    ...
                    eta_seconds = (elapsed / state.global_step) * (
...
class UnslothTrainer:
    ...
    def train(
        ...
        # NOTE: Not using train_on_responses_only due to compatibility issues with SFTTrainer
        # The model will learn from the full sequence, but the structured format
        # (### Instruction: ... ### Response: ...) helps it understand the task
        # This is actually standard practice for instruction fine-tuning
        logger.info("\n⚠️  Training on full sequences (instruction + response)")
        logger.info("The Alpaca format helps the model learn the instruction-following pattern\n")
...
def main():
    ...
    trainer.train(
        ...
        batch_size=8,  # Using 4 for better GPU utilization
        learning_rate=2e-4,
    )
