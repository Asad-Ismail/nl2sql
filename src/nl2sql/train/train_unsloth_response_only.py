...
def format_columns(example):
    ...
    return {"instruction": combined_instruction, "output": sql}
...
def load_and_prepare_datasets(dataset_configs: List[DatasetConfig], tokenizer) -> tuple:
    ...
    formatted_dataset = mixed_dataset.map(
        lambda x: formatting_prompts_func(x, tokenizer), batched=True, desc="Formatting"
    )
...
class UnslothTrainer:
    ...
    def train(
        ...
        trainer = train_on_responses_only(
            trainer, instruction_part="### Instruction:", response_part="### Response:"
        )
