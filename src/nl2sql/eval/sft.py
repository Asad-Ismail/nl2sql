...
class FinetunedModelEvaluator:
    ...
    def evaluate(
        self,
        output_dir: str = "results/finetuned",
        ...
        # Overall metrics
        metrics = {"total": 0, "valid": 0, "matched": 0, "exact_match": 0}

        # Metrics by complexity
        complexity_metrics = defaultdict(
            lambda: {"total": 0, "valid": 0, "matched": 0, "exact_match": 0}
        )
        ...
        for i, item in enumerate(tqdm(data, desc="Evaluating")):
            ...
            try:
                ...
                gen_valid, gen_error, gen_results = execute_sql(generated_sql, db_path)
                ...
                if gen_valid and gold_valid:
                    results_match, comparison_feedback = compare_results(gen_results, gold_results)
                elif not gen_valid:
                ...
                if (i + 1) % print_every == 0:
                    ...
                    print(
                        f"   Valid SQL: {metrics['valid']}/{metrics['total']} ({100*metrics['valid']/metrics['total']:.1f}%)"
                    )
                    print(
                        f"   Results Match: {metrics['matched']}/{metrics['total']} ({100*metrics['matched']/metrics['total']:.1f}%)"
                    )
                    print(
                        f"   Exact Match: {metrics['exact_match']}/{metrics['total']} ({100*metrics['exact_match']/metrics['total']:.1f}%)"
                    )
                    print()
    ...
    def _generate_report(self, metrics, complexity_metrics, error_patterns, output_dir):
        ...
        print(
            f"  Results Match: {metrics['matched']} ({100*metrics['matched']/metrics['total']:.2f}%)"
        )
        print(
            f"  Exact Match: {metrics['exact_match']} ({100*metrics['exact_match']/metrics['total']:.2f}%)\n"
        )
...
- Exact Match: {metrics['exact_match']} ({100*metrics['exact_match']/metrics['total']:.2f}%)"""
        ...
        for category, stats in sorted(complexity_metrics.items()):
            if stats["total"] > 0:
                ...
                complexity_section += (
                    f"- Valid: {stats['valid']} ({100*stats['valid']/stats['total']:.2f}%)\n"
                )
...
def main():
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="models/nl2sql-unsloth/checkpoint-10800",
        help="Path to fine-tuned model",
    )
    parser.add_argument(
        "--database-dir",
        type=str,
        default="database/spider_data/database",
        help="Path to Spider database directory",
    )
    parser.add_argument(
        "--output", type=str, default="results/finetuned", help="Output directory for results"
    )
    parser.add_argument(
        "--num-samples", type=int, default=None, help="Number of samples to evaluate (default: all)"
    )
    parser.add_argument(
        "--print-every", type=int, default=1, help="Print intermediate results every N examples"
    )
    parser.add_argument(
        "--print-prompts",
        action="store_true",
    ...
    # Run evaluation
    evaluator = FinetunedModelEvaluator(model_path=args.model, database_dir=args.database_dir)
