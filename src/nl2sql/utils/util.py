...
def compare_results(generated_results: Optional[List], 
                   gold_results: Optional[List]) -> Tuple[bool, str]:
    """
...
def calculate_metrics(results: List[Dict]) -> Dict[str, Any]:
    ...
    return {
        ...
        "result_match_pct": (result_match_count / total_examples * 100) if total_examples > 0 else 0.0,
...
def print_metrics(metrics: Dict[str, Any], method_name: str = ""):
    ...
    print(f"Valid SQL:                {metrics['valid_sql_count']:,} ({metrics['valid_sql_pct']:.1f}%)")
    print(f"Results Match Gold:       {metrics['result_match_count']:,} ({metrics['result_match_pct']:.1f}%)")
...
def print_comparison(idx: int, question: str, gen_sql: str, gold_sql: str, 
                    gen_results: Optional[List], gold_results: Optional[List], 
                    results_match: bool, is_valid: bool) -> None:
    """
...
def save_evaluation_results(results: Any, output_dir: str, 
...
def save_evaluation_summary(metrics: Dict, output_dir: str, 
...
def generate_markdown_report(metrics: Dict[str, Any], 
                            ...
                            model_name: str = "",
                            dataset_name: str = "",
                            additional_sections: Optional[Dict[str, str]] = None) -> str:
    """
