...
class SyntheticDataGenerator:
    ...
    def generate_variations(self, example: Dict, n_variations: int = 5) -> List[Dict]:
        ...
        for para in paraphrases[1:]:  # Skip original
            variations.append({**example, "question": para, "augmentation": "paraphrase"})

        # 2. Schema shuffling
        try:
            new_sql, new_schema = self.shuffler.shuffle_schema(
                example["sql"], example.get("schema", {}), seed=hash(example["question"])
            )
        ...
        if simplified_sql != example["sql"] and self.validator.is_valid_sql(simplified_sql):
            variations.append({**example, "sql": simplified_sql, "augmentation": "simplify"})

        return variations[: n_variations + 1]

    def augment_dataset(
        self, input_file: str, output_file: str, multiplier: int = 3, validate: bool = True
    ):
        """
