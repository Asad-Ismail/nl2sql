...
class SQLDialectValidator:
    ...
    POSTGRES_SPECIFIC = ["RETURNING", "ILIKE", "SIMILAR TO", "::", "LATERAL", "GENERATE_SERIES"]
    ...
    def is_duplicate(self, example: Dict) -> Tuple[bool, str]:
        ...
        if q_hash in self.seen_pairs:
            ...
            if new_priority > existing_priority:
                ...
                return (
                    True,
                    f"Duplicate{conflict_msg} (replaced {existing.get('source_dataset', 'unknown')} with higher priority)",
                )
                ...
                if len(sql) < len(existing_sql):
                    ...
                    return (
                        True,
                        f"Duplicate (tie - kept shorter SQL from {example.get('source_dataset', 'unknown')})",
                    )
                else:
                    return (
                        True,
                        f"Duplicate (tie - kept existing from {existing.get('source_dataset', 'unknown')})",
                    )
            else:
                ...
                return (
                    True,
                    f"Duplicate{conflict_msg} (kept higher priority from {existing.get('source_dataset', 'unknown')})",
                )
...
class UnslothDatasetPreparer:
    ...
    def prepare_training_datasets(self) -> Dict[str, List[Dict]]:
        ...
        print(
            f"  Conflicts resolved: {self.stats['conflicts_resolved']:,} (same question, different SQL)"
        )
    ...
    def print_summary(self, config: Dict):
        ...
        for name, stats in self.stats["per_dataset"].items():
            ...
            loaded = stats["loaded"]
            print(
                f"  {name:.<20} {kept:>7,} kept from {loaded:,} loaded "
